...
def fold3d(input, stride=1, use_padding=True, reduce='mean', std=1.7, dists=None, external_weights=None):  # noqa
    ...
    if reduce == 'sum':
        ...
        kt_s, kt_e=max(0,kt//2-2),min(kt,kt//2+3)
        kh_s, kh_e=max(0,kh//2-2),min(kh,kh//2+3)
        kw_s, kw_e=max(0,kw//2-2),min(kw,kw//2+3)
        to = {'device': input.device, 'dtype': input.dtype}
        weights = 1e-2*torch.ones((1,1,kt, kh, kw,1,1,1),**to)  # the epsilon is to avoid dividing by zero
        weights[:,:,kt_s:kt_e,kh_s:kh_e,kw_s:kw_e,:,:,:]=1  # the center5 pixels
        output = _fold3d_sum(input * weights, stride, use_padding)
        if use_padding and (st, sh, sw) == (1, 1, 1):
            norm = weights.sum()
        else:
        ...
        to = {'device': input.device, 'dtype': input.dtype}
        weights = 1e-2*torch.ones((1,1,kt, kh, kw,1,1,1),**to)  # the epsilon is to avoid dividing by zero
        weights[:,:,kt//2,kh//2,kw//2,:,:,:]=1  # the center5 pixels
        output = _fold3d_sum(input * weights, stride, use_padding)
        if use_padding and (st, sh, sw) == (1, 1, 1):
            norm = weights.sum()
        else:
            ...
            norm[norm == 0] = 1
        output = output / norm
    elif reduce == 'weighted_mean_and_external_weights':
        loc_weights = _get_weights_fold3d_weighted_mean(input, kt, kh, kw, st, sh, sw, std)
        combined_weights=loc_weights*external_weights
        output = _fold3d_sum(input * combined_weights, stride, use_padding)
        if use_padding and (st, sh, sw) == (1, 1, 1):
            assert False, f'not yet written'
            norm = combined_weights.sum()
        else:
            # loc_weights = loc_weights.expand(1, 1, kt, kh, kw, t, h, w)
            ############ before norm: combined_weights shoud be [1,1,1,13,13,4,468,628]
            norm = _fold3d_sum(combined_weights, stride, use_padding)
            ############ after norm: norm should be [1, 1, 4, 480, 640]
            norm[norm == 0] = 1
        output = output / norm
    elif reduce == 'external_weights':
        kt_s, kt_e=max(0,kt//2-2),min(kt,kt//2+3)
        kh_s, kh_e=max(0,kh//2-2),min(kh,kh//2+3)
        kw_s, kw_e=max(0,kw//2-2),min(kw,kw//2+3)
        to = {'device': input.device, 'dtype': input.dtype}
        loc_weights = 1e-2*torch.ones((1,1,kt, kh, kw,1,1,1),**to)  # the epsilon is to avoid dividing by zero
        loc_weights[:,:,kt_s:kt_e,kh_s:kh_e,kw_s:kw_e,:,:,:]=1  # the center5 pixels
        combined_weights=loc_weights*external_weights
        output = _fold3d_sum(input * combined_weights, stride, use_padding)
        if use_padding and (st, sh, sw) == (1, 1, 1):
            assert False, f'not yet written'
...
def _get_weights_fold3d_weighted_mean(input, kt, kh, kw, st, sh, sw, std):
    ...
    weights = weights/torch.max(weights)  # when one axis' size is 1, normalizes max back to 1.
    return weights.view(1, 1, kt, kh, kw, 1, 1, 1)
