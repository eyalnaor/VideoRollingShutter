...
parser.add_argument('--input_dir', type = str, default = '', help='frame input directory, recursive')
parser.add_argument('--result_dir', type = str, default = '', help='output home directory')
parser.add_argument('--result_suffix', type = str, default = '', help='name to add to folder')
parser.add_argument('--reverse_order', type = str, default = 'False', choices = ['False','True'], help='reverse order of pairs')
parser.add_argument('--rot', type = int, default=0, choices = [0,90,180,270], help ='roation. Notice that 90/270 reverse roles of RS axis')
parser.add_argument('--flip', type = str, default='none', choices = ['none','hor','ver','hor_ver'], help='flips. Notice that ver flip flips RS direction to bottom-->up')
parser.add_argument('--first_frame_to_do', type=int, default=0, help='in each folder, which is the first frame to use. If -1, does all')
parser.add_argument('--num_frames_to_do', type=int, default=1, help='in each folder, how many pairs to do. If start is -1, does all and ignores this')
parser.add_argument('--num_interpolated_frames', type=int, default=None, help='if not None, specifies number of frames to create for each image, instead of 1/frames in time_step')
parser.add_argument('--save_augmented', type = str, default = 'False', choices = ['False','True'], help='save augmented outputs in adition to unaugmented')
