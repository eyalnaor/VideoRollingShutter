...
import resize_right_right.interp_methods_right as interp_methods
...
def resize(input,
    ...
    if clamp is not None:
        assert len(clamp) == 2, f'in resize, clip is not of len==2. clip: {clamp}'
        output = torch.clamp(output, clamp[0], clamp[1])
